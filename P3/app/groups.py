...
def create() -> Union[str, Response, Tuple[Response, int], Tuple[str, int]]:
    ...
    if request.method == 'POST':
        ...
        sign_up_price = request.form.get('sign_up_price', type)
        if not website:
            website = ''
        if not min_age:
            min_age = '1'
        ...
        db.execute(
            """
            INSERT INTO Groups (group_name, group_description, contact_email, website_url, min_age,
            max_age, sign_up_price, register_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
